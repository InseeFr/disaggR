...
summary.praislm <- function(object, ...) {
  ...
  TAB <- cbind(
    Estimate = coef(object), StdErr = serr, t.value = tval,
  ...
  pm <- rbind(
    matrix(do.call(c, Box.test(object$residuals, lag = 1, type = "Ljung-Box")[c("statistic", "p.value")]),
      dimnames = list("residuals", c("statistic", "p.value")), nrow = 1L
    ),
    if (rho != 0) {
      matrix(do.call(c, Box.test(object$residuals.decorrelated, lag = 1, type = "Ljung-Box")[c("statistic", "p.value")]),
  ...
  res <- list(
    call = object$call, coefficients = TAB, r.squared = r.squared,
...
#' @export
as.list.twoStepsBenchmark <- function(x, ...) {
  structure(x@.Data,
    names = names(x)
  )
}
...
#' @export
as.list.threeRuleSmooth <- function(x, ...) {
  structure(x@.Data,
    names = names(x)
  )
}
...
#' @export
setMethod("Ops",
  signature = c("disaggR", "vector"),
  function(e1, e2) callGeneric(as.ts(e1), e2)
)
#' @export
setMethod("Ops",
  signature = c("vector", "disaggR"),
  function(e1, e2) callGeneric(e1, as.ts(e2))
)
#' @export
setMethod("Ops",
  signature = c("disaggR", "missing"),
  function(e1, e2) callGeneric(as.ts(e1))
)
#' @export
setMethod("Ops",
  signature = c("ts", "disaggR"),
  function(e1, e2) callGeneric(e1, as.ts(e2))
)
#' @export
setMethod("Ops",
  signature = c("disaggR", "ts"),
  function(e1, e2) callGeneric(as.ts(e1), e2)
)
#' @export
setMethod("Ops",
  signature = c("disaggR", "disaggR"),
...
outliers.twoStepsBenchmark <- function(object, as.ts = FALSE) {
  ...
  if (is.null(outliers)) {
    ...
    model.list(object)$hfserie[, names(outliers),
      drop = FALSE
    ]
  } else {
    outliers
...
outliers.praislm <- function(object, as.ts = FALSE) {
  ...
  if (is.null(outliers)) {
    ...
    model.list(object)$X[, names(outliers),
      drop = FALSE
    ]
  } else {
    outliers
