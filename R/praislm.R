...
praislm <- function(X, y, include.rho, include.differenciation, set_coefficients, cl) {
  ...
  if (length(set_coefficients) == 0L) {
    names(set_coefficients) <- character()
  ...
  if (any(is.na(match_set))) {
    stop("These names of the set.coeff argument are not found : ",
      paste(names(set_coefficients)[is.na(match_set)]),
      call. = FALSE
    )
  }
