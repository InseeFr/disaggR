...
in_sample.praislm <- function(object, type = "changes") {
  ...
  y_lagged <- lag(y, k = -1)
  predicted_diff <- {
    if (m$include.differenciation) {
      fitted(object) + autocor
    } else {
      fitted(object) + autocor - y_lagged
    }
  }
...
in_disaggr_notctb <- function(object, type) {
  ...
  series <- switch(type,
    ...
    "levels-rebased" =
      ts(
        t(t(series) /
          series[which(apply(!(series == 0) & !(is.na(series)), 1L, all))[1L], ]) * 100,
        start = start(series),
...
in_disaggr_ctb.threeRuleSmooth <- function(object) {
  ...
  series <- cbind(
    (benchmark / lag(benchmark, -1) - 1) * 100,
    0,
    0
  )
...
in_disaggr.twoStepsBenchmark <- function(object, type = "changes") {
  if (type == "contributions") {
    in_disaggr_ctb(object)
  } else {
    in_disaggr_notctb(object, type)
  }
}
...
in_revisions.twoStepsBenchmark <- function(object, object_old, type = "changes") {
  if (!inherits(object_old, "twoStepsBenchmark") &&
    !inherits(object_old, "threeRuleSmooth")) {
    stop("old_object must be a twoStepsBenchmark or a threeRuleSmooth", call. = FALSE)
  }
...
in_scatter_impl <- function(object, type) {
  ...
  if (type == "levels" && m$include.differenciation) {
    stop("The model is differencied hence the scatterplot should be in changes",
      call. = FALSE
    )
  }
  ...
  structure(series,
    ...
    abline =
      c(
        constant = {
          if (!m$include.differenciation && type == "changes") {
            0
          } else {
            as.numeric(coefficients(object)["constant"])
          }
        },
...
in_scatter.threeRuleSmooth <- function(object, type = "levels") {
  ...
  benchmark_clean_win <- switch_window(
    m$start.benchmark,
    m$end.benchmark,
    tsp(m$lfserie)
  )
...
distance.tscomparison <- function(x, p = 2) {
  ...
  res <- switch(attr(x, "func"),
    ...
    in_disaggr = {
      if (identical(attr(x, "type"), "contributions")) {
        x[, "Smoothed part"] + x[, "Trend"]
      } else {
        x[, "Benchmark"] - ts_from_tsp(
          rowSums(x[, colnames(x) != "Benchmark", drop = FALSE]),
          tsp(x)
        )
      }
    },
    in_revisions = {
      if (identical(attr(x, "type"), "contributions")) {
        stop("The distance method doesn't support revisions of contributions", call. = FALSE)
      } else {
        x[, "Benchmark"]
      }
    }
  ...
  if (p == Inf) {
    max(abs(res), na.rm = TRUE)
  } else {
    mean(abs(res)^p, na.rm = TRUE)^(1 / p)
  }
}
