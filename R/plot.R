...
default_col_pal <- function(object) {
  if (identical(attr(object, "type"), "contributions")) {
    function(n) suppressWarnings(brewer.pal(n, "Set2"))[seq_len(n)]
  } else if (identical(attr(object, "func"), "in_scatter")) {
    colorRampPalette(colors = c("#2B6788", "#CBCBCB", "#90503F"), space = "Lab")
  } else {
    function(n) suppressWarnings(brewer.pal(n, "Set1"))[seq_len(n)]
  }
}
...
default_lty_pal <- function(object) {
  if (identical(attr(object, "func"), "in_scatter")) {
    seq_len
  } else {
    types <- c(
      "solid", "22", "42", "44",
...
plot_init <- function(xmin, xmax, ymin, ymax, xlab, ylab,
  ...
  if (abline.x) {
    ...
    abline(
      v = (floor(xlim[1L] %||% xmin + verysmall) + 1L):
...
plot_init_x <- function(x, xlab, ylab, main, ...) {
  ...
  plot_init(
    xmin = tspx[1L], xmax = tspx[2L] + deltat(x),
...
draw_axes <- function(timex, cex.axis) {
  ...
  if (is.null(timex)) {
    ...
    axis(
      side = 1L, at = year + 0.5, labels = year, tick = FALSE,
...
window_default <- function(x, start, end) {
  ...
  non_na_vals <- {
    if (NCOL(x) == 1L) {
      ...
      apply(x, 1L, function(x) !all(is.na(x)))
    }
  }
...
plotts <- function(x, show.legend, col, lty,
  ...
  draw_axes(
    if (identical(attr(x, "func"), "in_scatter")) NULL else timex_win,
    cex.axis
  )
...
plot_with_lf <- function(x, xlab, ylab,
  ...
  if (show.legend) {
    legend("bottomleft",
      legend = c(serie_name, "Low-frequency serie"),
      col = col, lty = lty, horiz = TRUE, bty = "n", cex = 0.8
    )
  }
...
default_theme_ggplot <- function(object, start, end, show.legend, xlab, ylab, mar) {
  ...
  ggplot2::`%+replace%`(
    classic,
    ggplot2::theme(
      axis.title.x = if (is.null(xlab)) ggplot2::element_blank() else classic$axis.title.x,
      axis.title.y = if (is.null(ylab)) ggplot2::element_blank() else classic$axis.title.y,
      plot.margin = if (is.null(mar)) {
        classic$plot.margin
      } else {
        ggplot2::margin(mar[3L], mar[4L],
          mar[1L], mar[2L],
          unit = "pt"
        )
      },
  ...
  switch(type,
    ...
    bar = g + ggplot2::geom_bar(ggplot2::aes(fill = Variables, group = !!group), stat = "identity", show.legend = show.legend) +
      ggplot2::stat_summary(
        fun = sum, geom = "line", colour = "black",
...
geom_path_scatter <- function(object, i, lty, show.legend) {
  ...
  df <- data.frame(
    Time = as.numeric(time(object))[-1L],
  ...
  ggplot2::geom_segment(
    data = df,
    ggplot2::aes(
      x = `High-frequency serie`, y = `Low-frequency serie`,
      xend = xend, yend = yend,
      colour = Time, group = i
    ),
    linetype = lty,
...
ggscatter <- function(object, show.legend, theme, start, end, xlab, ylab,
  ...
  if (!is.null(attr(object, "abline"))) {
    g <- g + ggplot2::geom_abline(
      intercept = attr(object, "abline")["constant"],
...
function_if_it_isnt_one <- function(f) {
  if (is.function(f)) {
    ...
    if (length(f) == 1L) {
      eval(bquote(function(n) rep(.(f), n)))
    } else {
      eval(bquote(function(n) .(f)[1L:n]))
    }
  }
}
...
autoplot_with_lf <- function(object, xlab, ylab,
  ...
  ggplotts(
    object = rbind(hfdf, lfdf), show.legend = show.legend,
...
autoplot.tscomparison <- function(object, xlab = NULL, ylab = NULL,
  ...
  if (type_label == "Contributions") {
    ...
    switch(attr(object, "func"),
      in_revisions = {
        ggplotts(
          object = object, show.legend = show.legend,
