...
clean_tsp <- function(x) {
  s <- start(x)
  if (length(s) == 2L) {
    ts(x, start = s, frequency = frequency(x))
  } else if (as.integer(frequency(x)) != frequency(x)) {
    stop("The frequencies must be integers", call. = FALSE)
  } else {
    stop("Incorrect time-serie phase", call. = FALSE)
  }
}
...
extend_tsp <- function(tsphf, lffreq) {
  ...
  c(
    floor(tsphf[1L] * lffreq + ts.eps) / lffreq,
