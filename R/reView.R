...
info_switch <- function(mainout_choice) {
  switch(mainout_choice,
    "Scatter plot" = {
      shiny::HTML(
        "These scatter plots display the relationship between",
    ...
    "In-sample predictions" = {
      shiny::HTML(
        "These plots display in-sample predictions produced with",
    ...
    "Comparison benchmark/input" = {
      shiny::HTML(
        "These plots compare the input high-frequency series with",
    ...
    "Revisions" = {
      shiny::HTML(
        "These plots display the differences between the former",
        "benchmark and the newer one."
      )
    },
    "Benchmark plot" = {
      shiny::HTML(
        "Two-steps benchmarks bend a time series with a",
    ...
    "Benchmark summary" = {
      shiny::HTML(
        "The portmanteau test here is of lag 1. If the test is",
...
get_clean_wins <- function(benchmark) {
  ...
  win_benchmark <- switch_window(
    m$start.benchmark,
    m$end.benchmark,
    tsp(m$lfserie)
  )
  win_coeffcalc <- switch_window(
    m$start.coeff.calc,
    m$end.coeff.calc,
    tsp(m$lfserie)
  )
  win_domain <- switch_window(
    m$start.domain,
    m$end.domain,
    tsp(m$hfserie)
  )
...
plotOutBrushAndRender <- function(object, plotswin, output, output_name, ns,
  ...
  output[[output_name]] <- {
    if (is.null(tryCatch(object(), error = function(e) NULL))) {
      NULL
    } else {
      shiny::renderPlot(plot(object(),
        start = plotswin()[1L],
        end = plotswin()[2L],
        ...
      ))
...
presets <- list(
  include.differenciation = c(TRUE, TRUE, FALSE, FALSE, FALSE, FALSE),
...
clean_set_coeff <- function(set_hfserie, new_bn_external_setter) {
  ...
  set_outliers <- set_coeff[intersect(
    names(set_coeff),
    names(outliers(new_bn_external_setter))
  )]


  if (length(set_outliers) == 0L) {
    set_hfserie
  } else {
...
set_inputs_to_default <- function(session, new_bn_external_setter) {
  ...
  if (length(model$set.const) != 0) {
    ...
    shiny::updateCheckboxInput(session, "setconst_button", value = FALSE)
  }
}
...
set_plots_window_with_brush <- function(session, input, old_bn) {
  ...
  shiny::updateSliderInput(session, "plotswin",
    value = c(
      round(tsplf[3L] * (input$brush$xmin - tsplf[1L])) + tsplf[1L],
    ...
    shiny::mainPanel(
      ...
      shiny::fluidRow(
        shiny::tags$style(type = "text/css", paste0(
          ".", ns("mainoutwithtitle"), cssmainoutwithtitle, "\n",
        ...
        shiny::column(
          ...
          shiny::radioButtons(ns("mainout_choice"), NULL,
            choices = c(
              "Scatter plot", "In-sample predictions", "Comparison benchmark/input",
        ...
        shiny::column(
          1,
          shiny::actionButton(ns("infobtn"),
            label = NULL, icon = shiny::icon("circle-info"),
            class = "btn-success"
          )
        ),
        align = "center"
  ...
  shiny::fluidRow(
    shiny::column(12,
      ...
      shiny::fluidRow(
        ...
        shiny::column(3, shiny::actionButton(ns("Reset"), "Reset",
          width = "100%",
          class = "btn-warning"
        )),
        shiny::column(3, shiny::actionButton(ns("Quit"), "Quit",
          width = "100%",
          class = "btn-danger"
        )),
...
reView_ui_module <- function(id) {
  ns <- shiny::NS(id)
  shiny::navbarPage(
    title = shiny::textOutput(ns("titlenavbar")),
    windowTitle = "reView",
    id = ns("menu"),
    selected = "Presets",
...
summary_table_html <- function(presets_list, old_bn, distance_p, ns = NULL, selected_preset_tab2 = NULL, compare = NULL) {
  ...
  shiny::tags$table(
    do.call(
      shiny::tags$tr,
      c(
        list(shiny::tags$td(colspan = 2)),
        lapply(1:6, function(n) {
          shiny::tags$th(
            link_if_in_shiny(paste0("model", n, "_actionlink"), paste("Model", n), ns,
              style = paste(
    ...
    shiny::tags$tr(
      shiny::tags$th("Distance",
        rowspan = if (is.null(old_bn)) 2 else 3
      ),
      shiny::tags$th(
        "In-sample predictions (lf changes)",
        format_table_row(
          vapply(
            presets_list,
            function(x) {
    ...
    shiny::tags$tr(shiny::tags$th(
      "Benchmark contributions (hf changes)",
      format_table_row(
        vapply(
          presets_list,
          function(x) {
    ...
    shiny::tags$tr(
      shiny::tags$th("Portmanteau", rowspan = 2),
    ...
    shiny::tags$tr(
      shiny::tags$th("Constant", rowspan = 2),
      shiny::tags$th("Value"),
      format_table_row(
        vapply(
          summ,
      ...
      shiny::fluidRow(
        shiny::column(
          ...
          shiny::div(shiny::plotOutput(ns("model1_plot"), click = ns("model1_plotclick"), height = "33%"),
            ...
            shiny::plotOutput(ns("model5_plot"), click = ns("model5_plotclick"), height = "33%"),
            class = ns("presetplot")
          )
        ),
        shiny::column(
          ...
          shiny::div(shiny::plotOutput(ns("model2_plot"), click = ns("model2_plotclick"), height = "33%"),
            ...
            shiny::plotOutput(ns("model6_plot"), click = ns("model6_plotclick"), height = "33%"),
            class = ns("presetplot")
          )
        )
      )
      ...
      shiny::div(
        shiny::isolate(shiny::renderUI({ # for the table to be independent of the input distance
          ...
          summary_table_html(
            presets_list(),
            old_bn(),
            if (!shiny::isTruthy(input$distance_p)) {
              2L
            } else {
              switch(input$distance_p,
                Manhattan = 1L,
                Euclidean = 2L,
                Max = Inf
              )
        ...
        do.call(
          ...
          c(
            ...
            Map(
              ...
              c(
                list(shiny::tags$td(
                  shiny::radioButtons(ns("distance_p"), NULL,
  ...
  shiny::moduleServer(
    ...
    function(input, output, session) {
      ...
      output$firstTabOutput <- shiny::renderUI({
        reView_server_tab1_switch(
          input, output, session, presets_list, old_bn, selected_preset_tab2,
          compare
        )
      })
      ...
      lapply(1L:6L, function(n) {
        output[[paste0("model", n, "_plot")]] <- shiny::renderPlot({
          plot(in_sample(presets_list()[[n]]),
            ...
            font.main = if (isTRUE(get_preset(old_bn()) == n) && compare()) {
              4
            } else {
              2
            }
    ...
    "Comparison benchmark/input" = {
      shiny::fluidRow(
        shiny::column(
          ...
          title,
          shiny::div(
            plotOutBrushAndRender(
              shiny::reactive(in_disaggr(benchmark(),
                type = "levels-rebased"
              )),
              plotswin,
              output,
    ...
    "Revisions" = {
      shiny::fluidRow(
        shiny::column(
          ...
          shiny::div(
            plotOutBrushAndRender(
              shiny::reactive(in_revisions(benchmark(), old_bn(),
                type = "levels"
              )),
              plotswin,
              output,
...
reView_server_tab2_switch <- function(input, output, new_bn, old_bn, ns, compare) {
  ...
  if (compare && !(mainout_choice %in% c("Comparison benchmark/input", "Revisions"))) {
    ...
    shiny::fluidRow(shiny::column(12, reView_server_tab2_switch_impl(new_bn, mainout_choice, plotswin, output, "mono", ns, old_bn),
      style = boxstyle
    ), style = lrmargins)
  }
}
...
reView_server_tab3 <- function(id, old_bn, new_bn, hfserie_name, lfserie_name, compare) {
  shiny::moduleServer(
    ...
    function(input, output, session) {
      ...
      old_call_text <- shiny::reactive(if (compare()) {
        get_benchmark_call(old_bn(), hfserie_name(), lfserie_name())
      } else {
        "No model to compare."
      })
        ...
        shiny::showModal(
          shiny::modalDialog(
            ...
            if (input$Copymade) {
              ...
              "Sorry but the browser blocked the keyboard !"
            },
            easyClose = TRUE,
            footer = NULL
          ),
...
reView_server_module <- function(id, old_bn, new_bn_external_setter, hfserie_name, lfserie_name, compare,
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    selected_preset_tab1 <- reView_server_tab1(
      "reViewtab1", old_bn, new_bn_external_setter, selected_preset_tab2,
      compare
    )

    # tab 2 : Modify

    new_bn <- reView_server_tab2(
      "reViewtab2",
    ...
    reset <- reView_server_tab3(
      "reViewtab3", old_bn, new_bn,
      hfserie_name, lfserie_name,
      compare
    )
...
rePort.reViewOutput <- function(object, output_file = NULL,
  ...
  if (object$compare) {
    ...
    file.copy(system.file("rmd/report_nocompare.Rmd", package = "disaggR"), temp_rmd, overwrite = TRUE)
  }
  rmarkdown::render(temp_rmd,
    output_file = temp_html,
    params = list(
      new_bn = object$benchmark,
    ...
    output_format = rmarkdown::html_document(
      css = system.file("rmd/report.css", package = "disaggR"),
      theme = NULL
    ),
    quiet = TRUE,
    ...
