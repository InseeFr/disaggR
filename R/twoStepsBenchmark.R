split_outlier_names <- function(outlier_strings) {
  ...
  if (any(lengths(str) == 0L)) {
    stop("The outlier names can't be interpreted (see ?twoStepsBenchmark)",
      call. = FALSE
    )
  }
    ...
    ts(
      vapply(
        ...
        function(serie) {
          ...
          c(
            rep(0, round((tspser[1L] - minmax[1L]) * hffreq)),
            as.numeric(serie),
            rep(
              switch(attr(serie, "type"),
                AO = 0,
...
interpret_outliers <- function(outliers, lffreq, tsphf) {
  ...
  if (!inherits(outliers, "list")) {
    stop("The outliers must be a named list (see ?twoStepsBenchmark)",
      call. = FALSE
    )
  }

  if (is.null(names(outliers))) {
    stop("The outliers list must have names (see ?twoStepsBenchmark)",
      call. = FALSE
    )
  }
  ...
  outliers_ts_list <-
    Map(
      function(splitted_name, vect) {
        if (length(vect) %% ratio != 0L) {
          stop("The outlier vector must be of length k * hf/lf",
            call. = FALSE
          )
        }
  ...
  if (length(valplaces) != 0) {
    ...
    if (lastval != length(lfresiduals)) {
      lfresiduals[(lastval + 1L):length(lfresiduals)] <-
        residuals_extrap_sequence(
          lfresiduals[lastval - 1L],
          lfresiduals[lastval],
          rho,
...
twoStepsBenchmark_impl <- function(hfserie, lfserie,
  ...
  regresults <- regression_estimation(
    hfserie, lfserie,
    include.differenciation, include.rho,
    start.coeff.calc, end.coeff.calc,
    set_coefficients, cl
  ...
  res <- list(
    benchmarked.serie = window(rests, start = tsp_cropped[1L], end = tsp_cropped[2L], extend = TRUE),
...
twoStepsBenchmark <- function(hfserie, lfserie,
                              ...
                              outliers = NULL, ...) {
  if (!is.ts(lfserie) || !is.ts(hfserie)) {
    stop("Not a ts object",
      call. = FALSE
    )
  }
  ...
  if (is.matrix(hfserie) &&
    ...
    ncol(hfserie) != 1L) {
    stop("The high-frequency mts must have column names", call. = FALSE)
  }
  ...
  constant <- get_constant_indic(
    NROW(hfserie), tsphf[3L], tsplf[3L],
  ...
  hfserie <- ts(
    matrix(c(constant, hfserie, outliers_mts),
      nrow = NROW(hfserie)
    ),
    start = tsphf[1L],
    frequency = tsphf[3L],
  ...
  if (anyDuplicated(colnames(hfserie))) {
    stop("Invalid colnames for hfserie",
      call. = FALSE
    )
  }

  res <- twoStepsBenchmark_impl(
    hfserie, lfserie,
    include.differenciation, include.rho,
