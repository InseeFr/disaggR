...
We use monthly, seasonnaly and working days adjusted (SA-WDA) turnover index (base 100 in 2015) as an indicator for monthly household actual final consumption in current M euros, benchmarked on the corresponding annual account, known up to 2021.
...
Let first use the most basic model, in levels and without constant : 

```{r, eval=FALSE}
benchmark <- twoStepsBenchmark(turnover_catering,consumption_catering)
```

We can compare the indicator and the estimated QNA series :
...
There is thus a significant contribution of the smoothed part to the QNA evolution :
```{r}
...
While there is a lot of ways to build this outlier indicator, a convenient way is derive it from the usual indicator. In this example, we see that the turnover index is close to 130 before and after the lockdowns. Thus the distance to 130 is a good indicator of the intensity of the crisis :
...
```{r, eval=FALSE}
benchmark_out <- twoStepsBenchmark(turnover_catering,consumption_catering_out, 
...
```{r, eval=FALSE}
benchmark_out_2020 <- twoStepsBenchmark(turnover_catering_2020,consumption_catering_out_2020, 
                                    outliers = list(AO2020 = outlier)
                                    set.coeff = list(AO2020 = 14)
                                        )
```
```{r}
coefficients(summary(benchmark_out))
...
This is a broad assumption, but the resulting estimates for April 2020 (1574 M€) is closer than the estimates without outlier (3085 M€) from the final estimates using annual accounts for 2020 and 2021 and outliers (2161 M€). And the 14 value for $b$ is not so far from the final estimated 8 value. 

One could have anticipated the annual account reasoning that total final consumption will drop less than turnover in April, as it include government consumption in hotels (for some social assistance policies that continued during lockdown) and exclude collective firms and factories restaurants, totally closed during lockdown, thus going for a slighlty lower $b$ coefficient, very close to the final estimate. 
