...
test_that("rate extrap works", {
  ...
  expect_equal(
    rate_extrap(lfserie, delta_rate = 0.2),
  ...
  expect_equal(
    rate_extrap(lfserie, delta_rate = 0.5),
...
test_that("threeRuleSmooth works", {
  expect_snapshot(
    print(as.ts(threeRuleSmooth(turnover, construction)),
      digits = 4L
    ),
    cran = FALSE
  )
  expect_equal(
    attributes(model.list(threeRuleSmooth(turnover, construction))$hfserie)$dim,
    c(245L, 1L)
  )
  expect_snapshot(
    print(
  ...
  expect_true(all(abs(aggregate(smooth1$smoothed.rate * smooth1$hfserie.as.weights,
    ...
    smooth1$lfrate) < 10^-8))
  expect_true(all(abs(aggregate(
    window(as.ts(smooth1),
      start = c(2001, 4),
      end = c(2016, 3),
      extend = TRUE
    ),
...
test_that("errors", {
  ...
  expect_error(
    threeRuleSmooth(ts(rep(0, 120), frequency = 12, start = 2000), construction),
    "Every hfserie aggregation value is equal to zero"
  )
  hfserie <- turnover
  hfserie[1L:12L] <- 0
  expect_error(
    threeRuleSmooth(hfserie, construction),
    "There is a zero"
  )
...
test_that("ts eps", {
  ...
  expect_identical(
    as.ts(threeRuleSmooth(turnover_tspmodif, construction_tspmodif)),
    as.ts(threeRuleSmooth(turnover, construction))
  )
  expect_identical(
    as.ts(threeRuleSmooth(turnover_tspmodif, construction)),
    as.ts(threeRuleSmooth(disaggR::turnover, disaggR::construction))
  )
  expect_identical(
    as.ts(threeRuleSmooth(turnover, construction_tspmodif)),
    as.ts(threeRuleSmooth(disaggR::turnover, disaggR::construction))
  )
