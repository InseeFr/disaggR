test_that("switch window", {
  ...
  tspex <- tsp(tsexample)
  expect_identical(
    switch_window(2010, 2011.4, tspex),
    c(2010, 2011.4)
  )
  expect_identical(
    switch_window(c(2010, 4), NULL, tspex),
    c(2010.25, tspex[2L])
  )
  expect_identical(
    switch_window(NULL, 2011.4, tspex),
    c(tspex[1L], 2011.4)
  )
})
...
test_that("display_vector", {
  ...
  expect_identical(
    display_vector(c(a = 1)),
    "c(a=1)"
  )
  expect_identical(
    display_vector(c(a = 1, b = 2)),
    "c(a=1,b=2)"
  )
  expect_identical(
    display_outliers(list(a = c(0.1, 0.1))),
    "list(a=c(0.1,0.1))"
  )
  expect_identical(
    display_outliers(list(
      a = c(0.1, 0.1),
      b = 0.3
    )),
...
test_that("get clean wins", {
  ...
  expect_identical(
    get_clean_wins(benchmark),
    list(
      benchmark = c(2005, 2017),
...
test_that("names args changes the name on rePort", {
  ...
  temp_dir <- tempdir()
  temp_html <- tempfile("test", temp_dir, ".html")
  expect_message(rePort(benchmark,
    output_file = temp_html, launch.browser = TRUE,
    hfserie_name = "testhf",
    lfserie_name = "testlf"
  ))
  expect_true(file.exists(temp_html))
...
test_that("reView and rePort return an error on multivariate benchmarks", {
  expect_error(
    rePort(twoStepsBenchmark(cbind(turnover, turnover_catering), construction)),
    "univariate benchmarks"
  )
  expect_error(
    reView(twoStepsBenchmark(cbind(turnover, turnover_catering), construction)),
    "univariate benchmarks"
  )
})
...
test_that("reView-withoutset", {
  ...
  app$set_inputs(
    `reView-reViewtab1-model1_plotclick` = 10L,
    allow_no_input_binding_ = TRUE
  )
  slidercoeffcalc <- app$wait_for_value(
    input = "reView-reViewtab2-coeffcalc",
    timeout = 5000
  )
  ...
  expect_equal(
    app$wait_for_value(input = "reView-reViewtab2-plotswin"),
    c(2003, 2014)
  )
  ...
  expect_equal(
    app$wait_for_value(input = "reView-reViewtab2-mainout_choice"),
    "Scatter plot"
  )
...
test_that("reView-outliers", {
  ...
  app$set_inputs(`reView-menu` = "Export")
  expect_equal(
    app$wait_for_value(output = "reView-reViewtab3-newcall"),
  ...
  expect_equal(
    app$wait_for_value(output = "reView-reViewtab3-oldcall"),
    paste("twoStepsBenchmark(",
      "hfserie = turnover,",
      "lfserie = construction,",
      "include.differenciation = FALSE,",
