...
test_that("colname is taken even with ncol = 1", {
  ...
  expect_identical(
    names(coef(twoStepsBenchmark(turnover_modif, construction)))[2L],
    "test"
  )
})
...
test_that("mts works", {
  ...
  bn <- twoStepsBenchmark(mat, construction,
    include.differenciation = TRUE,
...
test_that("windows and extraps works", {
  ...
  expect_equal(
    as.numeric(window(extrap, start = c(2011, 2), end = c(2011, 2), extend = TRUE)),
    rho(bn)
  )
  expect_equal(
    as.numeric(window(extrap, start = c(2019, 2))),
    rep(rho(bn), 4)
  )
  ...
  expect_equal(
    as.numeric(window(extrap, start = c(2011, 3), end = c(2011, 3), extend = TRUE)),
    rho(bn)
  )
  expect_equal(
    as.numeric(window(extrap, start = c(2019, 2))),
  ...
  extrap <- asp / lag(asp, -1)
  expect_equal(
    as.numeric(window(extrap, start = c(2015, 2))),
    rho(bn)
  )
...
test_that("errors", {
  ...
  expect_error(
    twoStepsBenchmark(correct_names_mts, construction,
      set.coeff = c(test = 1)
    ),
  ...
  expect_error(
    twoStepsBenchmark(ts(matrix(rnorm(900, 0, 100), ncol = 3), start = c(2000, 1), freq = 12) %>%
      `unname`(), construction),
    regexp = "column names"
  )
  expect_error(
    twoStepsBenchmark(
      ts(matrix(rnorm(900, 0, 100), ncol = 3), start = c(2000, 1), freq = 12) %>%
        `colnames<-`(c("a", "b", "c")),
      construction,
      set.coeff = 1:4
    ),
  ...
  expect_error(
    twoStepsBenchmark_impl(turnover, construction,
  ...
  expect_error(
    twoStepsBenchmark(
      hfserie = ts(diffinv(rnorm(240, 1, 1)), start = 2010.1, freq = 12),
  ...
  expect_error(
    twoStepsBenchmark(
      hfserie = ts(diffinv(rnorm(240, 1, 1)), start = 2010, freq = 12),
...
test_that("annualBenchmark", {
  ...
  trimts <- ts(diffinv(rnorm(36, 12, 1)), start = 2010, freq = 4)
  expect_error(
    annualBenchmark(mensualts, trimts),
    "annual time series"
  )
  expect_snapshot(
    print(
      annualBenchmark(turnover, construction,
        end.coeff.calc = 2018
      ),
      digits = 4L
    ),
  ...
  expect_equal(
    as.ts(annualBenchmark(turnover, construction,
      end.coeff.calc = 2018
    )),
    as.ts(twoStepsBenchmark(turnover, construction,
      end.coeff.calc = 2018,
  ...
  expect_equal(
    as.ts(annualBenchmark(mensualts, annualts, end.coeff.calc = 2019)),
...
test_that("ts eps", {
  ...
  expect_identical(
    as.ts(twoStepsBenchmark(turnover_tspmodif, construction_tspmodif)),
    as.ts(twoStepsBenchmark(disaggR::turnover, disaggR::construction))
  )
  expect_identical(
    as.ts(twoStepsBenchmark(turnover_tspmodif, construction)),
    as.ts(twoStepsBenchmark(disaggR::turnover, disaggR::construction))
  )
  expect_identical(
    as.ts(twoStepsBenchmark(turnover, construction_tspmodif)),
    as.ts(twoStepsBenchmark(disaggR::turnover, disaggR::construction))
  )
...
test_that("test outliers", {
  expect_error(
    twoStepsBenchmark(turnover, construction, outliers = list(AO2020 = rep(0.1, 12))),
    "perfect rank"
  )
  expect_error(
    twoStepsBenchmark(turnover, construction, outliers = c(AO2008 = rep(0.1, 12))),
    "named list"
  )
  expect_error(
    twoStepsBenchmark(turnover, construction, outliers = list(rep(0.1, 12))),
    "must have names"
  )
  expect_error(
    twoStepsBenchmark(turnover, construction, outliers = list(Aqdqsd = rep(0.1, 12))),
    "be interpreted"
  )
  expect_error(
    twoStepsBenchmark(turnover, construction, outliers = list(AO2008 = rep(0.1, 11))),
    "length"
  )
  expect_error(
    twoStepsBenchmark(turnover, construction, outliers = list(AO2008 = rep(0.1, 13))),
    "length"
  )

  expected <- as.ts(twoStepsBenchmark(
    cbind(
      turnover,
  ...
  object <- model.list(twoStepsBenchmark(window(turnover, start = c(2003, 3)),
    ...
    outliers = list(AO2006T1 = rep(0.1, 12))
  ))$hfserie[, "AO2006T1"]
  expected <- ts(c(
    rep(0, 34L),
    rep(0.1, 12L),
    rep(0, 161)
  ), start = c(2003, 3), frequency = 12)
  ...
  object <- model.list(twoStepsBenchmark(mensualts,
    trimts,
    outliers = list(AO2011T2 = rep(0.1, 6L))
  ))$hfserie[, "AO2011T2"]
  expected <- ts(c(
    rep(0, 15L),
    rep(0.1, 6L),
