...
test_that("in_disaggr works", {
  ...
  class(obtained) <- {
    if (R.version$major >= 4 && R.version$minor >= 3) {
      c("tscomparison", "mts", "ts", "matrix", "array")
    } else {
      c("tscomparison", "mts", "ts", "matrix")
    }
  }
...
test_that("in scatter works", {
  ...
  attr(expected, "abline") <- c(
    constant = as.numeric(coefficients(benchmark)["constant"]),
  ...
  expected <- ts(
    matrix(
      c(
        window(reg$model.list$y, start = 2005, end = 2017, extend = TRUE),
        window(aggregate(turnover), start = 2005, end = 2017, extend = TRUE)
      ),
      ncol = 2, dimnames = list(NULL, c(
        "Low-frequency serie",
        "High-frequency serie (regression)"
      ))
    ),
  ...
  attr(expected, "func") <- "in_scatter"
  attr(expected, "abline") <- c(
    constant = as.numeric(coefficients(reg)["constant"]),
...
test_that("error in", {
  ...
  expect_error(
    in_disaggr(benchmark, type = "aaza"),
    "The type argument of in_disaggr"
  )
  expect_error(
    in_revisions(twoStepsBenchmark(turnover, construction),
  ...
  expect_error(
    in_scatter(threeRuleSmooth(turnover, construction),
      type = "contributions"
    ),
    "The type argument of in_scatter"
  )
  expect_error(
    in_revisions(benchmark, "nothing important"),
  ...
  expect_error(
    in_sample(threeRuleSmooth(turnover, construction)),
...
test_that("distance", {
  ...
  expect_error(
    distance(in_revisions(benchmark, benchmark, type = "contributions")),
...
test_that("outliers", {
  ...
  construction_corr <-
    construction -
    window(
      aggregate(coefficients(benchmark)["AO2007"] *
  ...
  expected <- ts(
    matrix(
      c(
        window(construction_corr, start = 2005, end = 2017, extend = TRUE),
        window(aggregate(turnover), start = 2005, end = 2017, extend = TRUE)
      ),
      ncol = 2, dimnames = list(NULL, c(
        "Low-frequency serie",
  ...
  expected <- ts(
    matrix(
      c(
        window(construction_out_corr, start = 2002, end = 2019, extend = TRUE),
  ...
  expected <- ts(
    matrix(
      c(
        window(construction_out_corr, start = 2002, end = 2014, extend = TRUE),
        window(window(aggregate(turnover), start = 2002, end = 2014, extend = TRUE),
...
test_that("in_revisions with different outliers", {
  ...
  expect_equal(
    ts_from_tsp(rowSums(res), tsp(res)),
    rev
  )
